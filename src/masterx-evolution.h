...
#ifndef MASTERX_EVOLUTION_H
#define MASTERX_EVOLUTION_H
...
public:
    ...
    std::map<uint256, CEvolutionProposalBroadcast> mapSeenMasterXEvolutionProposals;
    std::map<uint256, CEvolutionVote> mapSeenMasterXEvolutionVotes;
    std::map<uint256, CEvolutionVote> mapOrphanMasterXEvolutionVotes;
    ...
    void ClearSeen() {
        mapSeenMasterXEvolutionProposals.clear();
        mapSeenMasterXEvolutionVotes.clear();
    ...
    void Clear(){
        ...
        mapSeenFinalizedEvolutions.clear();
        mapSeenFinalizedEvolutionVotes.clear();
        mapOrphanMasterXEvolutionVotes.clear();
    ...
    inline void SerializationOp(Stream& s, Operation ser_action, int nType, int nVersion) {
        READWRITE(mapSeenMasterXEvolutionProposals);
        READWRITE(mapSeenMasterXEvolutionVotes);
        READWRITE(mapSeenFinalizedEvolutions);
        READWRITE(mapSeenFinalizedEvolutionVotes);
        READWRITE(mapOrphanMasterXEvolutionVotes);
...
private:
    // critical section to protect the inner data structures
    mutable CCriticalSection cs;
    bool fAutoChecked; //If it matches what we see, we'll auto vote for it (masterx only)
...
public:
    ...
    void AutoCheck();
    //total redux paid out by this evolution
    CAmount GetTotalPayout();
    //vote on this finalized evolution as a masterx
    void SubmitVote();
...
//
// CFinalizedEvolutionVote - Allow a masterx node to vote and broadcast throughout the network
//
...
//
// Evolution Proposal : Contains the masterx votes for each evolution
//
...
//
// CEvolutionVote - Allow a masterx node to vote and broadcast throughout the network
//
