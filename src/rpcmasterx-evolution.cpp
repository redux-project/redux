...
Value gmevolution(const Array& params, bool fHelp)
{
    ...
    if (fHelp  ||
        ...
        throw runtime_error(
                ...
                "  vote               - Vote on a Redux initiative/evolution\n"
                "  getvotes           - Show current masterx evolutions\n"
                "  getinfo            - Show current masterx evolutions\n"
    ...
    if(strCommand == "prepare")
    {
        ...
        if (params.size() != 7)
            throw runtime_error("Correct usage is 'gmevolution prepare proposal-name url payment_count block_start redux_address monthly_payment_redux'");
    ...
    if(strCommand == "submit")
    {
        ...
        if (params.size() != 8)
            throw runtime_error("Correct usage is 'gmevolution submit proposal-name url payment_count block_start redux_address monthly_payment_redux fee_tx'");
        ...
        if(!masterxSync.IsBlockchainSynced()){
            return "Must wait for client to sync with masterx network. Try again in a minute or so.";            
        }
        ...
        evolution.mapSeenMasterXEvolutionProposals.insert(make_pair(evolutionProposalBroadcast.GetHash(), evolutionProposalBroadcast));
    ...
    if(strCommand == "vote-many")
    {
        ...
        BOOST_FOREACH(CMasterXConfig::CMasterXEntry mne, masterxConfig.getEntries()) {
            ...
            std::vector<unsigned char> vchMasterXSignature;
            ...
            if(!spySendSigner.SetKey(mne.getPrivKey(), errorMessage, keyMasterX, pubKeyMasterX)){
                failed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", "MasterX signing error, could not set key correctly: " + errorMessage));
                resultsObj.push_back(Pair(mne.getAlias(), statusObj));
                continue;
            }

            CMasterX* pgm = gmineman.Find(pubKeyMasterX);
            if(pgm == NULL)
            {
                failed++;
                statusObj.push_back(Pair("result", "failed"));
                statusObj.push_back(Pair("errorMessage", "Can't find masterx by pubkey"));
                resultsObj.push_back(Pair(mne.getAlias(), statusObj));
                continue;
            }
            ...
            if(evolution.UpdateProposal(vote, NULL, strError)) {
                evolution.mapSeenMasterXEvolutionVotes.insert(make_pair(vote.GetHash(), vote));
                vote.Relay();
                success++;
    ...
    if(strCommand == "vote")
    {
        ...
        if(pgm == NULL)
        {
            return "Failure to find masterx in list : " + activeMasterX.vin.ToString();
        }

        CEvolutionVote vote(activeMasterX.vin, hash, nVote);
        if(!vote.Sign(keyMasterX, pubKeyMasterX)){
...
Value gmevolutionvoteraw(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 6)
        throw runtime_error(
                "gmevolutionvoteraw <masterx-tx-hash> <masterx-tx-index> <proposal-hash> <yes|no> <time> <vote-sig>\n"
    ...
    if(pgm == NULL)
    {
        return "Failure to find masterx in list : " + vin.ToString();
    }
...
Value gmfinalevolution(const Array& params, bool fHelp)
{
    ...
    if(strCommand == "vote")
    {
        ...
        CFinalizedEvolutionVote vote(activeMasterX.vin, hash);
        if(!vote.Sign(keyMasterX, pubKeyMasterX)){
            return "Failure to sign.";
        }
