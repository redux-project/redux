...
class CTxIn;
class CStealthXPool;
class CStealthXSigner;
class CMasterXVote;
class CBitcoinAddress;
class CStealthXQueue;
class CStealthXBroadcastTx;
...
#define MASTERX_ACCEPTED                    1
#define MASTERX_REJECTED                    0
#define MASTERX_RESET                       -1

#define STEALTHX_QUEUE_TIMEOUT                 30
#define STEALTHX_SIGNING_TIMEOUT               15

// used for anonymous relaying of inputs/outputs/sigs
#define STEALTHX_RELAY_IN                 1
#define STEALTHX_RELAY_OUT                2
#define STEALTHX_RELAY_SIG                3

static const int64_t STEALTHX_COLLATERAL = (0.01*COIN);
static const int64_t STEALTHX_POOL_MAX = (999.99*COIN);

extern CStealthXPool spySendPool;
extern CStealthXSigner spySendSigner;
extern std::vector<CStealthXQueue> vecStealthXQueue;
extern std::string strMasterXPrivKey;
extern map<uint256, CStealthXBroadcastTx> mapStealthXBroadcastTxes;
extern CActiveMasterX activeMasterX;

/** Holds an StealthX input
 */
...
// A clients transaction in the stealthx pool
class CStealthXEntry
{
public:
    bool isSet;
    ...
    /// Add entries to use for StealthX
    ...
    bool IsExpired()
    {
        return (GetTime() - addedTime) > STEALTHX_QUEUE_TIMEOUT;// 120 seconds
    }
};


/**
 * A currently inprogress StealthX merge and denomination information
 */
class CStealthXQueue
{
public:
    ...
    /** Sign this StealthX transaction
     *  \return true if all conditions are met:
     *     1) we have an active MasterX,
     *     2) we have a valid MasterX private key,
    ...
    /// Is this StealthX expired?
    bool IsExpired()
    {
        return (GetTime() - time) > STEALTHX_QUEUE_TIMEOUT;// 120 seconds
    }

    /// Check if we have a valid MasterX address
    bool CheckSignature();

};

/** Helper class to store StealthX transaction (tx) information.
 */
class CStealthXBroadcastTx
{
public:
...
{
public:
    /// Is the inputs associated with this public key? (and there is 1000 REDUX - checking if valid masterx)
...
/** Used to keep track of current status of StealthX pool
 */
class CStealthXPool
{
private:
    mutable CCriticalSection cs_stealthx;

    std::vector<CStealthXEntry> entries; // MasterX/clients entries
    ...
    bool sessionFoundMasterX; //If we've found a compatible MasterX
...
public:
    ...
    CMasterX* pSubmittedToMasterX;
    int sessionDenom; //Users must submit an denom matching this
    int cachedNumBlocks; //used for the overview screen

    CStealthXPool()
    {
        /* StealthX uses collateral addresses to trust parties entering the pool
    ...
    /** Process a StealthX message using the StealthX protocol
     * \param pfrom
     * \param strCommand lower case command string; valid values are:
     *        Command  | Description
     *        -------- | -----------------
     *        dsa      | StealthX Acceptable
     *        dsc      | StealthX Complete
     *        dsf      | StealthX Final tx
     *        dsi      | StealthX vIn
     *        dsq      | StealthX Queue
     *        dss      | StealthX Signal Final Tx
     *        dssu     | StealthX status update
     *        dssub    | StealthX Subscribe To
     * \param vRecv
     */
    void ProcessMessageStealthX(CNode* pfrom, std::string& strCommand, CDataStream& vRecv);

    void InitCollateralAddress(){
        SetCollateralAddress(Params().StealthXPoolDummyAddress());
    }
    ...
    {
        if (fMasterX && (newState == POOL_STATUS_ERROR || newState == POOL_STATUS_SUCCESS)){
            LogPrint("stealthx", "CStealthXPool::UpdateState() - Can't set state to ERROR or SUCCESS as a MasterX. \n");
            return;
        }

        LogPrintf("CStealthXPool::UpdateState() == %d | %d \n", state, newState);
        if(state != newState){
    ...
    /// Passively run StealthX in the background according to the configuration in settings (only for QT)
    bool DoAutomaticDenominating(bool fDryRun=false);
    bool PrepareStealthXDenominate();

    /// Check for process in StealthX
    void Check();
    ...
    /// As a client, send a transaction to a MasterX to start the denomination process
    void SendStealthXDenominate(std::vector<CTxIn>& vin, std::vector<CTxOut>& vout, int64_t amount);
    /// Get MasterX updates about the progress of StealthX
