    ...
    model(0)
{
    ...
    ui->treeWidget->setColumnWidth(COLUMN_STEALTHX_ROUNDS, 88);
...
void CoinControlDialog::viewItemChanged(QTreeWidgetItem* item, int column)
{
    if (column == COLUMN_CHECKBOX && item->text(COLUMN_TXHASH).length() == 64) // transaction hash is 64 characters (this means its a child node, so its not a parent node in tree mode)
    {
        ...
        if (item->checkState(COLUMN_CHECKBOX) == Qt::Unchecked)
            ...
            CTxIn vin(outpt);
            int rounds = pwalletMain->GetInputStealthXRounds(vin);
            if(coinControl->useStealthX && rounds < nStealthXRounds) {
                QMessageBox::warning(this, windowTitle(),
                    tr("Non-anonymized input selected. <b>StealthX will be disabled.</b><br><br>If you still want to use StealthX, please deselect all non-nonymized inputs first and then check StealthX checkbox again."),
                    QMessageBox::Ok, QMessageBox::Ok);
                coinControl->useStealthX = false;
            }
        }
...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    if (nQuantity > 0)
    {
        ...
        if (nPayAmount > 0)
        {
            ...
            // SS Fee = overpay
            if(coinControl->useStealthX && nChange > 0)
            {
                nPayFee += nChange;
                nChange = 0;
            }
...
void CoinControlDialog::updateView()
{
    ...
    BOOST_FOREACH(PAIRTYPE(QString, vector<COutput>) coins, mapCoins)
    {
        ...
        BOOST_FOREACH(const COutput& out, coins.second)
        {
            ...
            if(rounds >= 0) itemOutput->setText(COLUMN_STEALTHX_ROUNDS, strPad(QString::number(rounds), 11, " "));
            else itemOutput->setText(COLUMN_STEALTHX_ROUNDS, strPad(QString(tr("n/a")), 11, " "));
