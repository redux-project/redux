    ...
    fFeeMinimized(true)
{
    ...
    // Redux specific
    QSettings settings;
    if (!settings.contains("bUseStealthX"))
        settings.setValue("bUseStealthX", false);
    if (!settings.contains("bUseInstantX"))
        settings.setValue("bUseInstantX", false);
        
    bool useStealthX = settings.value("bUseStealthX").toBool();
    bool useInstantX = settings.value("bUseInstantX").toBool();
    if(fLiteMode) {
        ui->checkUseStealthX->setChecked(false);
        ui->checkUseStealthX->setVisible(false);
        ui->checkInstantX->setVisible(false);
        CoinControlDialog::coinControl->useStealthX = false;
        CoinControlDialog::coinControl->useInstantX = false;
    }
    else{
        ui->checkUseStealthX->setChecked(useStealthX);
        ui->checkInstantX->setChecked(useInstantX);
        CoinControlDialog::coinControl->useStealthX = useStealthX;
        CoinControlDialog::coinControl->useInstantX = useInstantX;
    }
    
    connect(ui->checkUseStealthX, SIGNAL(stateChanged ( int )), this, SLOT(updateDisplayUnit()));
...
void SendCoinsDialog::on_sendButton_clicked()
{
    ...
    if(ui->checkUseStealthX->isChecked()) {
        recipients[0].inputType = ONLY_DENOMINATED;
        ...
        strFee = QString(tr(
            "(stealthx requires this amount to be rounded up to the nearest %1)."
                                 ...
                                 const CAmount& watchBalance, const CAmount& watchUnconfirmedBalance, const CAmount& watchImmatureBalance)
{
    ...
    if(model && model->getOptionsModel())
    {
	    uint64_t bal = 0;
        QSettings settings;
        settings.setValue("bUseStealthX", ui->checkUseStealthX->isChecked());
	    if(ui->checkUseStealthX->isChecked()) {
...
void SendCoinsDialog::updateDisplayUnit()
{
    ...
    setBalance(model->getBalance(), model->getUnconfirmedBalance(), model->getImmatureBalance(), model->getAnonymizedBalance(),
                   model->getWatchBalance(), model->getWatchUnconfirmedBalance(), model->getWatchImmatureBalance());
    CoinControlDialog::coinControl->useStealthX = ui->checkUseStealthX->isChecked();
    coinControlUpdateLabels();
...
void SendCoinsDialog::coinControlUpdateLabels()
{
    ...
    ui->checkUseStealthX->setChecked(CoinControlDialog::coinControl->useStealthX);
