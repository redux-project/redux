...
//    if(secs < 25*60) // 90*60 for bitcoin but we are 4x times faster
    if(masterxSync.IsBlockchainSynced())
    {
        ...
        if(masterxSync.IsSynced()) {
            ...
            nAttempt = masterxSync.RequestedMasterXAttempt < MASTERX_SYNC_THRESHOLD ?
                        masterxSync.RequestedMasterXAttempt + 1 : MASTERX_SYNC_THRESHOLD;
            progress = nAttempt + (masterxSync.RequestedMasterXAssets - 1) * MASTERX_SYNC_THRESHOLD;
            progressBar->setMaximum(4 * MASTERX_SYNC_THRESHOLD);
            progressBar->setFormat(tr("Synchronizing additional data: %p%"));
            progressBar->setValue(progress);
        }

        strSyncStatus = QString(masterxSync.GetSyncStatus().c_str());
