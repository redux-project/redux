...
//    bool SelectCoins(int64_t nTargetValue, std::set<std::pair<const CWalletTx*,unsigned int> >& setCoinsRet, int64_t& nValueRet, const CCoinControl *coinControl = NULL, AvailableCoinsType coin_type=ALL_COINS, bool useIX = true) const;
    bool SelectCoinsDark(int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& setCoinsRet, int64_t& nValueRet, int nStealthXRoundsMin, int nStealthXRoundsMax) const;
    bool SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, int64_t& nValueRet, int nStealthXRoundsMin, int nStealthXRoundsMax);
    ...
    std::string PrepareStealthXDenominate(int minRounds, int maxRounds);
    int GenerateStealthXOutputs(int nTotalValue, std::vector<CTxOut>& vout);
    ...
    // get the StealthX chain depth for a given input
    int GetRealInputStealthXRounds(CTxIn in, int rounds) const;
    // respect current settings
    int GetInputStealthXRounds(CTxIn in) const;
...
public:
    ...
    CAmount GetAnonymizableCredit(bool fUseCache=true) const
    {
        ...
        for (unsigned int i = 0; i < vout.size(); i++)
        {
            ...
            if(fMasterX && vout[i].nValue == 1000*COIN) continue; // do not count MN-like outputs

            const int rounds = pwallet->GetInputStealthXRounds(vin);
            if(rounds >=-2 && rounds < nStealthXRounds) {
                nCredit += pwallet->GetCredit(txout, ISMINE_SPENDABLE);
                if (!MoneyRange(nCredit))
...
public:
    ...
    //Used with StealthX. Will return largest nondenom, then denominations, then very small inputs
    int Priority() const
    {
