...
void CEvolutionManager::CheckOrphanVotes()
{
    ...
    std::string strError = "";
    std::map<uint256, CEvolutionVote>::iterator it1 = mapOrphanMasterXEvolutionVotes.begin();
    while(it1 != mapOrphanMasterXEvolutionVotes.end()){
        if(evolution.UpdateProposal(((*it1).second), NULL, strError)){
            ...
            mapOrphanMasterXEvolutionVotes.erase(it1++);
        } else {
            ++it1;
        }
    }
...
CEvolutionDB::CEvolutionDB()
{
    ...
    strMagicMessage = "MasterXEvolution";
}
...
bool CEvolutionManager::IsEvolutionPaymentBlock(int nBlockHeight)
{
    ...
    /*
        If evolution doesn't have 5% of the network votes, then we should pay a masterx instead
    */
...
bool CEvolutionManager::HasNextFinalizedEvolution()
{
    ...
    if(masterxSync.IsEvolutionFinEmpty()) return true;
...
void CEvolutionManager::NewBlock()
{
    ...
    if (masterxSync.RequestedMasterXAssets <= MASTERX_SYNC_EVOLUTION) return;
...
void CEvolutionManager::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "gmvs") { //MasterX vote sync
        uint256 nProp;
        vRecv >> nProp;
        ...
        Sync(pfrom, nProp);
        LogPrintf("gmvs - Sent MasterX votes to %s\n", pfrom->addr.ToString());
    }

    if (strCommand == "mprop") { //MasterX Proposal
        CEvolutionProposalBroadcast evolutionProposalBroadcast;
        vRecv >> evolutionProposalBroadcast;

        if(mapSeenMasterXEvolutionProposals.count(evolutionProposalBroadcast.GetHash())){
            masterxSync.AddedEvolutionItem(evolutionProposalBroadcast.GetHash());
            return;
        }
        ...
        mapSeenMasterXEvolutionProposals.insert(make_pair(evolutionProposalBroadcast.GetHash(), evolutionProposalBroadcast));
    ...
    if (strCommand == "mvote") { //MasterX Vote
        CEvolutionVote vote;
        vRecv >> vote;
        vote.fValid = true;

        if(mapSeenMasterXEvolutionVotes.count(vote.GetHash())){
            masterxSync.AddedEvolutionItem(vote.GetHash());
            return;
        }

        CMasterX* pgm = gmineman.Find(vote.vin);
        if(pgm == NULL) {
            LogPrint("gmevolution", "mvote - unknown masterx - vin: %s\n", vote.vin.ToString());
            gmineman.AskForGM(pfrom, vote.vin);
            return;
        }


        mapSeenMasterXEvolutionVotes.insert(make_pair(vote.GetHash(), vote));
    ...
    if (strCommand == "fbs") { //Finalized Evolution Suggestion
        ...
        if(mapSeenFinalizedEvolutions.count(finalizedEvolutionBroadcast.GetHash())){
            masterxSync.AddedEvolutionItem(finalizedEvolutionBroadcast.GetHash());
            return;
        }
    ...
    if (strCommand == "fbvote") { //Finalized Evolution Vote
        ...
        CMasterX* pgm = gmineman.Find(vote.vin);
        if(pgm == NULL) {
            LogPrint("gmevolution", "fbvote - unknown masterx - vin: %s\n", vote.vin.ToString());
            gmineman.AskForGM(pfrom, vote.vin);
            return;
        }
...
void CEvolutionManager::ResetSync()
{
    ...
    std::map<uint256, CEvolutionProposalBroadcast>::iterator it1 = mapSeenMasterXEvolutionProposals.begin();
    while(it1 != mapSeenMasterXEvolutionProposals.end()){
...
void CEvolutionManager::Sync(CNode* pfrom, uint256 nProp, bool fPartial)
{
    ...
    */
    ...
    pfrom->PushMessage("ssc", MASTERX_SYNC_EVOLUTION_PROP, nInvCount);
    ...
    pfrom->PushMessage("ssc", MASTERX_SYNC_EVOLUTION_FIN, nInvCount);
    LogPrintf("CEvolutionManager::Sync - sent %d items\n", nInvCount);
...
// If masterx voted for a proposal, but is now invalid -- remove the vote
...
bool CEvolutionVote::Sign(CKey& keyMasterX, CPubKey& pubKeyMasterX)
{
    // Choose coins to use
...
bool CEvolutionVote::SignatureValid(bool fSignatureCheck)
{
    ...
    if(pgm == NULL)
    {
        LogPrint("gmevolution", "CEvolutionVote::SignatureValid() - Unknown MasterX - %s\n", vin.ToString());
        return false;
    }
...
//evaluate if we should vote for this. MasterX only
void CFinalizedEvolution::AutoCheck()
{
    ...
    if(!fMasterX || fAutoChecked) return;
...
void CFinalizedEvolution::SubmitVote()
{
    ...
    CFinalizedEvolutionVote vote(activeMasterX.vin, GetHash());
    if(!vote.Sign(keyMasterX, pubKeyMasterX)){
...
bool CFinalizedEvolutionVote::Sign(CKey& keyMasterX, CPubKey& pubKeyMasterX)
{
    // Choose coins to use
    CPubKey pubKeyCollateralAddress;
    CKey keyCollateralAddress;
...
bool CFinalizedEvolutionVote::SignatureValid(bool fSignatureCheck)
{
    ...
    if(pgm == NULL)
    {
        LogPrint("gmevolution", "CFinalizedEvolutionVote::SignatureValid() - Unknown MasterX\n");
        return false;
    }
...
std::string CEvolutionManager::ToString() const
{
    ...
    info << "Proposals: " << (int)mapProposals.size() <<
            ...
            ", Seen Evolutions: " << (int)mapSeenMasterXEvolutionProposals.size() <<
            ", Seen Evolution Votes: " << (int)mapSeenMasterXEvolutionVotes.size() <<
