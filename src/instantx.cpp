...
//step 2.) Top INSTANTX_SIGNATURES_TOTAL masterxs, open connect to top 1 masterx.
//         Send "txvote", CTransaction, Signature, Approve
//step 3.) Top 1 masterx, waits for INSTANTX_SIGNATURES_REQUIRED messages. Upon success, sends "txlock'
...
void ProcessMessageInstantX(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
    ...
    else if (strCommand == "txlvote") //InstantX Lock Consensus Votes
    {
        ...
        if(ProcessConsensusVote(pfrom, ctx)){
            //Spam/Dos protection
            /*
                MasterX will sometimes propagate votes before the transaction is known to the client.
            ...
            */
            if(!mapTxLockReq.count(ctx.txHash) && !mapTxLockReqRejected.count(ctx.txHash)){
                if(!mapUnknownVotes.count(ctx.vinMasterX.prevout.hash)){
                    mapUnknownVotes[ctx.vinMasterX.prevout.hash] = GetTime()+(60*10);
                }

                if(mapUnknownVotes[ctx.vinMasterX.prevout.hash] > GetTime() &&
                    mapUnknownVotes[ctx.vinMasterX.prevout.hash] - GetAverageVoteTime() > 60*10){
                        LogPrintf("ProcessMessageInstantX::ix - masterx is spamming transaction votes: %s %s\n",
                            ctx.vinMasterX.ToString().c_str(),
...
int64_t CreateNewLock(CTransaction tx)
{
    ...
    /*
        ...
        This prevents attackers from using transaction mallibility to predict which masterxs
        they'll use.
    */
...
void DoConsensusVote(CTransaction& tx, int64_t nBlockHeight)
{
    ...
    int n = gmineman.GetMasterXRank(activeMasterX.vin, nBlockHeight, MIN_INSTANTX_PROTO_VERSION);

    if(n == -1)
    {
        LogPrint("instantx", "InstantX::DoConsensusVote - Unknown MasterX\n");
        return;
    }

    if(n > INSTANTX_SIGNATURES_TOTAL)
    {
        LogPrint("instantx", "InstantX::DoConsensusVote - MasterX not in the top %d (%d)\n", INSTANTX_SIGNATURES_TOTAL, n);
        return;
    }
    /*
    ...
    */
    ...
    CConsensusVote ctx;
    ctx.vinMasterX = activeMasterX.vin;
...
{
    int n = gmineman.GetMasterXRank(ctx.vinMasterX, ctx.nBlockHeight, MIN_INSTANTX_PROTO_VERSION);

    CMasterX* pgm = gmineman.Find(ctx.vinMasterX);
    if(pgm != NULL)
        LogPrint("instantx", "InstantX::ProcessConsensusVote - MasterX ADDR %s %d\n", pgm->addr.ToString().c_str(), n);

    if(n == -1)
    {
        //can be caused by past versions trying to vote with an invalid protocol
        LogPrint("instantx", "InstantX::ProcessConsensusVote - Unknown MasterX\n");
        gmineman.AskForGM(pnode, ctx.vinMasterX);
        return false;
    }

    if(n > INSTANTX_SIGNATURES_TOTAL)
    {
        LogPrint("instantx", "InstantX::ProcessConsensusVote - MasterX not in the top %d (%d) - %s\n", INSTANTX_SIGNATURES_TOTAL, n, ctx.GetHash().ToString().c_str());
        return false;
    }

    if(!ctx.SignatureValid()) {
        LogPrintf("InstantX::ProcessConsensusVote - Signature invalid\n");
        // don't ban, it could just be a non-synced masterx
        gmineman.AskForGM(pnode, ctx.vinMasterX);
        return false;
    }
...
{
    return vinMasterX.prevout.hash + vinMasterX.prevout.n + txHash;
}
...
bool CConsensusVote::SignatureValid()
{
    ...
    if(pgm == NULL)
    {
        LogPrintf("InstantX::CConsensusVote::SignatureValid() - Unknown MasterX\n");
        return false;
    }

    if(!spySendSigner.VerifyMessage(pgm->pubkey2, vchMasterXSignature, strMessage, errorMessage)) {
...
bool CConsensusVote::Sign()
{
    ...
    //LogPrintf("signing privkey %s \n", strMasterXPrivKey.c_str());

    if(!spySendSigner.SetKey(strMasterXPrivKey, errorMessage, key2, pubkey2))
    {
        LogPrintf("CConsensusVote::Sign() - ERROR: Invalid masterxprivkey: '%s'\n", errorMessage.c_str());
        return false;
    }

    if(!spySendSigner.SignMessage(strMessage, errorMessage, vchMasterXSignature, key2)) {
        LogPrintf("CConsensusVote::Sign() - Sign message failed");
        return false;
    }

    if(!spySendSigner.VerifyMessage(pubkey2, vchMasterXSignature, strMessage, errorMessage)) {
...
bool CTransactionLock::SignaturesValid()
{
    ...
    {
        int n = gmineman.GetMasterXRank(vote.vinMasterX, vote.nBlockHeight, MIN_INSTANTX_PROTO_VERSION);

        if(n == -1)
        {
            LogPrintf("CTransactionLock::SignaturesValid() - Unknown MasterX\n");
            return false;
        }

        if(n > INSTANTX_SIGNATURES_TOTAL)
        {
            LogPrintf("CTransactionLock::SignaturesValid() - MasterX not in the top %s\n", INSTANTX_SIGNATURES_TOTAL);
            return false;
        }
