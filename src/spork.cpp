...
bool IsSporkActive(int nSporkID)
{
    ...
    if(mapSporksActive.count(nSporkID)){
        ...
        if(nSporkID == SPORK_7_MASTERX_SCANNING) r = SPORK_7_MASTERX_SCANNING_DEFAULT;
        if(nSporkID == SPORK_8_MASTERX_PAYMENT_ENFORCEMENT) r = SPORK_8_MASTERX_PAYMENT_ENFORCEMENT_DEFAULT;
        if(nSporkID == SPORK_9_MASTERX_EVOLUTION_ENFORCEMENT) r = SPORK_9_MASTERX_EVOLUTION_ENFORCEMENT_DEFAULT;
        if(nSporkID == SPORK_10_MASTERX_PAY_UPDATED_NODES) r = SPORK_10_MASTERX_PAY_UPDATED_NODES_DEFAULT;
...
bool CSporkManager::Sign(CSporkMessage& spork)
{
    ...
    {
        LogPrintf("CMasterXPayments::Sign - ERROR: Invalid masterxprivkey: '%s'\n", errorMessage);
        return false;
    }

    if(!spySendSigner.SignMessage(strMessage, errorMessage, spork.vchSig, key2)) {
        LogPrintf("CMasterXPayments::Sign - Sign message failed");
        return false;
    }

    if(!spySendSigner.VerifyMessage(pubkey2, spork.vchSig, strMessage, errorMessage)) {
        LogPrintf("CMasterXPayments::Sign - Verify message failed");
        return false;
    }
...
int CSporkManager::GetSporkIDByName(std::string strName)
{
    ...
    if(strName == "SPORK_7_MASTERX_SCANNING") return SPORK_7_MASTERX_SCANNING;
    if(strName == "SPORK_8_MASTERX_PAYMENT_ENFORCEMENT") return SPORK_8_MASTERX_PAYMENT_ENFORCEMENT;
    if(strName == "SPORK_9_MASTERX_EVOLUTION_ENFORCEMENT") return SPORK_9_MASTERX_EVOLUTION_ENFORCEMENT;
    if(strName == "SPORK_10_MASTERX_PAY_UPDATED_NODES") return SPORK_10_MASTERX_PAY_UPDATED_NODES;
...
std::string CSporkManager::GetSporkNameByID(int id)
{
    ...
    if(id == SPORK_7_MASTERX_SCANNING) return "SPORK_7_MASTERX_SCANNING";
    if(id == SPORK_8_MASTERX_PAYMENT_ENFORCEMENT) return "SPORK_8_MASTERX_PAYMENT_ENFORCEMENT";
    if(id == SPORK_9_MASTERX_EVOLUTION_ENFORCEMENT) return "SPORK_9_MASTERX_EVOLUTION_ENFORCEMENT";
    if(id == SPORK_10_MASTERX_PAY_UPDATED_NODES) return "SPORK_10_MASTERX_PAY_UPDATED_NODES";
