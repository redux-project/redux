...
/** 
 * Fees smaller than this (in nREDUX) are considered zero fee (for transaction creation)
...
// Recursively determine the rounds of a given input (How deep is the StealthX chain for a given input)
int CWallet::GetRealInputStealthXRounds(CTxIn in, int rounds) const
{
    ...
    if(wtx != NULL)
    {
        ...
        {
            LogPrint("stealthx", "GetInputStealthXRounds INSERTING %s\n", hash.ToString());
        ...
        if(nout >= wtx->vout.size())
        {
            ...
            LogPrint("stealthx", "GetInputStealthXRounds UPDATED   %s %3d %3d\n", hash.ToString(), nout, -4);
            return -4;
        }
        ...
        BOOST_FOREACH(CTxIn in2, wtx->vin)
        {
            if(IsMine(in2))
            {
                int n = GetRealInputStealthXRounds(in2, rounds+1);
...
int CWallet::GetInputStealthXRounds(CTxIn in) const {
    LOCK(cs_wallet);
    int realStealthXRounds = GetRealInputStealthXRounds(in, 0);
    return realStealthXRounds > nStealthXRounds ? nStealthXRounds : realStealthXRounds;
}
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int rounds = GetInputStealthXRounds(vin);
                fTotal += (float)rounds;
                fCount += 1;
            }
        }
...

    {
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                int rounds = GetInputStealthXRounds(vin);
                nTotal += pcoin->vout[i].nValue * rounds / nStealthXRounds;
            }
        }
    }
        ...
        for (map<uint256, CWalletTx>::const_iterator it = mapWallet.begin(); it != mapWallet.end(); ++it)
        {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++) {
                ...
                if(coin_type == ONLY_DENOMINATED) {
                    ...
                    found = !(fMasterX && pcoin->vout[i].nValue == 1000*COIN);
                } else if(coin_type == ONLY_NONDENOMINATED_NOT1000IFMN) {
                    if (IsCollateralAmount(pcoin->vout[i].nValue)) continue; // do not use collateral amounts
                    found = !IsDenominatedAmount(pcoin->vout[i].nValue);
                    if(found && fMasterX) found = pcoin->vout[i].nValue != 1000*COIN; // do not use Hot MN funds
                } else {
                    found = true;
                }
                if(!found) continue;
...
bool less_then_denom (const COutput& out1, const COutput& out2)
{
    ...
    bool found1 = false;
    bool found2 = false;
    BOOST_FOREACH(int64_t d, spySendDenominations) // loop through predefined denoms
    {
...
bool CWallet::SelectCoins(const CAmount& nTargetValue, set<pair<const CWalletTx*,unsigned int> >& setCoinsRet, CAmount& nValueRet, const CCoinControl* coinControl, AvailableCoinsType coin_type, bool useIX) const
{
    ...
    if (coinControl && coinControl->HasSelected())
    {
        BOOST_FOREACH(const COutput& out, vCoins)
        {
            ...
            if(coin_type == ONLY_DENOMINATED) {
                ...
                if(rounds < nStealthXRounds) continue;
            }
    ...
    //if we're doing only denominated, we need to round up to the nearest .1REDUX
    if(coin_type == ONLY_DENOMINATED) {
        // Make outputs by looping through denominations, from large to small
        BOOST_FOREACH(int64_t v, spySendDenominations)
        {
            BOOST_FOREACH(const COutput& out, vCoins)
            {
                if(out.tx->vout[out.i].nValue == v                                            //make sure it's the denom we're looking for
                    && nValueRet + out.tx->vout[out.i].nValue < nTargetValue + (0.1*COIN)+100 //round the amount up to .1REDUX over
                ){
                    CTxIn vin = CTxIn(out.tx->GetHash(),out.i);
                    int rounds = GetInputStealthXRounds(vin);
                    // make sure it's actually anonymized
...
bool CWallet::SelectCoinsByDenominations(int nDenom, int64_t nValueMin, int64_t nValueMax, std::vector<CTxIn>& vCoinsRet, std::vector<COutput>& vCoinsRet2, int64_t& nValueRet, int nStealthXRoundsMin, int nStealthXRoundsMax)
{
    vCoinsRet.clear();
    nValueRet = 0;
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        // masterx-like input should not be selected by AvailableCoins now anyway
        ...
        if(nValueRet + out.tx->vout[out.i].nValue <= nValueMax){
            ...
            int rounds = GetInputStealthXRounds(vin);
            if(rounds >= nStealthXRoundsMax) continue;
            if(rounds < nStealthXRoundsMin) continue;
...
bool CWallet::SelectCoinsDark(CAmount nValueMin, CAmount nValueMax, std::vector<CTxIn>& setCoinsRet, CAmount& nValueRet, int nStealthXRoundsMin, int nStealthXRoundsMax) const
{
    ...
    vector<COutput> vCoins;
    AvailableCoins(vCoins, true, coinControl, nStealthXRoundsMin < 0 ? ONLY_NONDENOMINATED_NOT1000IFMN : ONLY_DENOMINATED);
    ...
    BOOST_FOREACH(const COutput& out, vCoins)
    {
        ...
        if(fMasterX && out.tx->vout[out.i].nValue == 1000*COIN) continue; //masterx input
...
{
    return  nInputAmount != 0 && nInputAmount % STEALTHX_COLLATERAL == 0 && nInputAmount < STEALTHX_COLLATERAL * 5 && nInputAmount > STEALTHX_COLLATERAL;
}
...
bool CWallet::CreateCollateralTransaction(CMutableTransaction& txCollateral, std::string& strReason)
{
    ...
    */
    ...
    {
        strReason = "Error: StealthX requires a collateral transaction and could not locate an acceptable input!";
        return false;
    }
    ...
    if(nValueIn2 - STEALTHX_COLLATERAL - nFeeRet > 0) {
        //pay collateral charge in fees
        CTxOut vout3 = CTxOut(nValueIn2 - STEALTHX_COLLATERAL, scriptChange);
    ...
    BOOST_FOREACH(CTxIn v, txCollateral.vin) {
        if(!SignSignature(*this, v.prevPubKey, txCollateral, vinNumber, int(SIGHASH_ALL|SIGHASH_ANYONECANPAY))) {
            ...
            strReason = "CStealthXPool::Sign - Unable to sign collateral transaction! \n";
            return false;
        }
        vinNumber++;
    }
...
bool CWallet::GetEvolutionSystemCollateralTX(CTransaction& tx, uint256 hash, bool useIX)
{
    CWalletTx wtx;
    if(GetEvolutionSystemCollateralTX(wtx, hash, useIX)){
        tx = (CTransaction)wtx;
        return true;
    }
    return false;
...
bool CWallet::GetEvolutionSystemCollateralTX(CWalletTx& tx, uint256 hash, bool useIX)
{
    // make our change address
            ...
            while (true)
            {
                ...
                if (!SelectCoins(nTotalValue, setCoins, nValueIn, coinControl, coin_type, useIX))
                {
                    if(coin_type == ALL_COINS) {
                        ...
                        strFailReason = _("Unable to locate enough funds for this transaction that are not equal 1000 REDUX.");
                    } else if (coin_type == ONLY_NONDENOMINATED_NOT1000IFMN) {
                        strFailReason = _("Unable to locate enough StealthX non-denominated funds for this transaction that are not equal 1000 REDUX.");
                    } else {
                        strFailReason = _("Unable to locate enough StealthX denominated funds for this transaction.");
                        strFailReason += " " + _("StealthX uses exact denominated amounts to send funds, you might simply need to anonymize some more coins.");
                    }
...
string CWallet::PrepareStealthXDenominate(int minRounds, int maxRounds)
{
    if (IsLocked())
        return _("Error: Wallet locked, unable to create transaction!");

    if(spySendPool.GetState() != POOL_STATUS_ERROR && spySendPool.GetState() != POOL_STATUS_SUCCESS)
        if(spySendPool.GetEntriesCount() > 0)
            return _("Error: You already have pending entries in the StealthX pool");
    ...
    */
    if(minRounds >= 0){
        if (!SelectCoinsByDenominations(spySendPool.sessionDenom, 0.1*COIN, STEALTHX_POOL_MAX, vCoins, vCoins2, nValueIn, minRounds, maxRounds))
            return _("Error: Can't select current denominated inputs");
    }

    LogPrintf("PrepareStealthXDenominate - preparing stealthx denominate . Got: %d \n", nValueIn);
    ...
    */
    ...
    while(nStep < nStepsMax) {
        ...
        BOOST_FOREACH(int64_t v, spySendDenominations){
            ...
            bool fAccepted = false;
            if((spySendPool.sessionDenom & (1 << 0))      && v == ((100*COIN) +100000)) {fAccepted = true;}
            else if((spySendPool.sessionDenom & (1 << 1)) && v == ((10*COIN)  +10000)) {fAccepted = true;}
            else if((spySendPool.sessionDenom & (1 << 2)) && v == ((1*COIN)   +1000)) {fAccepted = true;}
            else if((spySendPool.sessionDenom & (1 << 3)) && v == ((.1*COIN)  +100)) {fAccepted = true;}
            if(!fAccepted) continue;
    ...
    if(spySendPool.GetDenominations(vOut) != spySendPool.sessionDenom) {
    ...
    spySendPool.SendStealthXDenominate(vCoinsResult, vOut, nValueIn - nValueLeft);
