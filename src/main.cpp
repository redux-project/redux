...
/** Fees smaller than this (in nREDUX) are considered zero fee (for relaying and mining)
                        ...
                        bool* pfMissingInputs, bool fRejectInsaneFee, bool ignoreFees)
{
...

    {
        ...
        if(mapStealthXBroadcastTxes.count(hash)) {
...
int64_t GetBlockValue(int nBits, int nHeight, const CAmount& nFees)
{
	...
	if(nHeight == 0) {
		...
		nSubsidy = 20060000; //20060000  всего монет за 1 год работы и к моменту перехода на новую версию кошелька.
		LogPrintf("First block: Transfer members balance %i REDUX\n", nSubsidy);
		nSubsidy *= COIN;
		/* 
		* Т.к. кошелек не находился до этого момента в публичном доступе было принято решение
		* не заниматься хард форком, а просто привести кошелек в исходное состояние, 
		* с переносом всех остатков со всех кошельков в 1 блок, тем самым гарантируя всем сохранность их монет.
		* При этом произведен переход на новый алгоритм шифрования X11.
		* Такой метод, на наш взгляд, позволяет избежать лишних надстроек в исходном коде и 
		* перезапустить блокчейн монеты с учетом всех последних разработок в этой области.
		*   int64 nSubsidy = 150 * COIN;
		*	if(nHeight < 1001){nSubsidy = 500 * COIN;}
		*	формула начисления предыдущей версии
		*/
...
int64_t GetMasterXPayment(int nHeight, int64_t blockValue)
{	
	int64_t ret = 0;
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    if(pindexPrev != NULL)
    {
        ...
        if(nHeight != 0){
            ...
            {
                mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                return state.DoS(100, error("CheckBlock() : Couldn't find masterx/evolution payment"));
            }
        } else {
...
bool ProcessNewBlock(CValidationState &state, CNode* pfrom, CBlock* pblock, CDiskBlockPos *dbp)
{
    ...
    if(!fLiteMode){
        if (masterxSync.RequestedMasterXAssets > MASTERX_SYNC_LIST) {
            spySendPool.NewBlock();
            masterxPayments.ProcessBlock(GetHeight()+10);
            evolution.NewBlock();
        }
    }
...
bool static AlreadyHave(const CInv& inv)
{
    ...
    case MSG_DSTX:
        return mapStealthXBroadcastTxes.count(inv.hash);
    case MSG_BLOCK:
    ...
    case MSG_MASTERX_WINNER:
        if(masterxPayments.mapMasterXPayeeVotes.count(inv.hash)) {
            masterxSync.AddedMasterXWinner(inv.hash);
            return true;
        }
        return false;
    case MSG_EVOLUTION_VOTE:
        if(evolution.mapSeenMasterXEvolutionVotes.count(inv.hash)) {
            masterxSync.AddedEvolutionItem(inv.hash);
            return true;
        }
        return false;
    case MSG_EVOLUTION_PROPOSAL:
        if(evolution.mapSeenMasterXEvolutionProposals.count(inv.hash)) {
            masterxSync.AddedEvolutionItem(inv.hash);
            return true;
        }
        return false;
    ...
    case MSG_EVOLUTION_FINALIZED:
        ...
        return false;
    case MSG_MASTERX_ANNOUNCE:
        if(gmineman.mapSeenMasterXBroadcast.count(inv.hash)) {
            masterxSync.AddedMasterXList(inv.hash);
            return true;
        }
        return false;
    case MSG_MASTERX_PING:
        return gmineman.mapSeenMasterXPing.count(inv.hash);
    }
    ...
    while (it != pfrom->vRecvGetData.end()) {
        ...
        const CInv &inv = *it;
        {
            ...
            else if (inv.IsKnownType())
            {
                ...
                if (!pushed && inv.type == MSG_MASTERX_WINNER) {
                    if(masterxPayments.mapMasterXPayeeVotes.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << masterxPayments.mapMasterXPayeeVotes[inv.hash];
                ...
                if (!pushed && inv.type == MSG_EVOLUTION_VOTE) {
                    if(evolution.mapSeenMasterXEvolutionVotes.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << evolution.mapSeenMasterXEvolutionVotes[inv.hash];
                ...
                if (!pushed && inv.type == MSG_EVOLUTION_PROPOSAL) {
                    if(evolution.mapSeenMasterXEvolutionProposals.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << evolution.mapSeenMasterXEvolutionProposals[inv.hash];
                ...
                if (!pushed && inv.type == MSG_MASTERX_ANNOUNCE) {
                    if(gmineman.mapSeenMasterXBroadcast.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << gmineman.mapSeenMasterXBroadcast[inv.hash];
                        pfrom->PushMessage("gmb", ss);
                        pushed = true;
                    }
                }

                if (!pushed && inv.type == MSG_MASTERX_PING) {
                    if(gmineman.mapSeenMasterXPing.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss << gmineman.mapSeenMasterXPing[inv.hash];
                ...
                if (!pushed && inv.type == MSG_DSTX) {       
                    if(mapStealthXBroadcastTxes.count(inv.hash)){
                        CDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
                        ss.reserve(1000);
                        ss <<
                            mapStealthXBroadcastTxes[inv.hash].tx <<
                            mapStealthXBroadcastTxes[inv.hash].vin <<
                            mapStealthXBroadcastTxes[inv.hash].vchSig <<
                            mapStealthXBroadcastTxes[inv.hash].sigTime;
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    else if (strCommand == "tx"|| strCommand == "dstx")
    {
        ...
        if(strCommand == "tx") {
            ...
            //these allow masterxs to publish a limited amount of free transactions
            ...
            {
                if(!pgm->allowFreeTx){
                    //multiple peers can send us a valid masterx transaction
                    if(fDebug) LogPrintf("dstx: MasterX sending too many transactions %s\n", tx.GetHash().ToString());
                    return true;
                }
                ...
                if(!spySendSigner.VerifyMessage(pgm->pubkey2, vchSig, strMessage, errorMessage)){
                    LogPrintf("dstx: Got bad masterx address signature %s \n", vin.ToString());
                    //pfrom->Misbehaving(20);
                    return false;
                }

                LogPrintf("dstx: Got MasterX transaction %s\n", tx.GetHash().ToString());

                ignoreFees = true;
                pgm->allowFreeTx = false;

                if(!mapStealthXBroadcastTxes.count(tx.GetHash())){
                    ...
                    mapStealthXBroadcastTxes.insert(make_pair(tx.GetHash(), dstx));
                }
            }
        }
    ...
    else
    {
        ...
        spySendPool.ProcessMessageStealthX(pfrom, strCommand, vRecv);
        gmineman.ProcessMessage(pfrom, strCommand, vRecv);
        evolution.ProcessMessage(pfrom, strCommand, vRecv);
        masterxPayments.ProcessMessageMasterXPayments(pfrom, strCommand, vRecv);
        ProcessMessageInstantX(pfrom, strCommand, vRecv);
        ProcessSpork(pfrom, strCommand, vRecv);
        masterxSync.ProcessMessage(pfrom, strCommand, vRecv);
    }
