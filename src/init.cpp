...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += "  -conf=<file>           " + strprintf(_("Specify configuration file (default: %s)"), "redux.conf") + "\n";
...
#ifndef WIN32
    strUsage += "  -pid=<file>            " + strprintf(_("Specify pid file (default: %s)"), "reduxd.pid") + "\n";
#endif
    ...
    strUsage += "  -port=<port>           " + strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 9667, 19667) + "\n";
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += "  -mintxfee=<amt>          " + strprintf(_("Fees (in REDUX/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"), FormatMoney(CWallet::minTxFee.GetFeePerK())) + "\n";
    strUsage += "  -paytxfee=<amt>          " + strprintf(_("Fee (in REDUX/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "                           redux (or specifically: stealthx, instantx, masterx, keepass, gmpayments, gmevolution)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        strUsage += ", qt";
    strUsage += ".\n";
#ifdef ENABLE_WALLET
    ...
    strUsage += "  -minrelaytxfee=<amt>   " + strprintf(_("Fees (in REDUX/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())) + "\n";
    ...
    strUsage += "  -litemode=<n>          " + strprintf(_("Disable all Redux Core specific functionality (MasterX, StealthX, InstantX, Evolution) (0-1, default: %u)"), 0) + "\n";

    strUsage += "\n" + _("MasterX options:") + "\n";
    strUsage += "  -masterx=<n>            " + strprintf(_("Enable the client to act as a masterx (0-1, default: %u)"), 0) + "\n";
    strUsage += "  -gmconf=<file>             " + strprintf(_("Specify masterx configuration file (default: %s)"), "masterx.conf") + "\n";
    strUsage += "  -gmconflock=<n>            " + strprintf(_("Lock masterxs from masterx configuration file (default: %u)"), 1) + "\n";
    strUsage += "  -masterxprivkey=<n>     " + _("Set the masterx private key") + "\n";
    strUsage += "  -masterxaddr=<n>        " + strprintf(_("Set external address:port to get to this masterx (example: %s)"), "128.127.106.235:9667") + "\n";
    ...
    strUsage += "\n" + _("StealthX options:") + "\n";
    strUsage += "  -enablestealthx=<n>          " + strprintf(_("Enable use of automated stealthx for funds stored in this wallet (0-1, default: %u)"), 0) + "\n";
    strUsage += "  -stealthxrounds=<n>          " + strprintf(_("Use N separate masterxs to anonymize funds  (2-8, default: %u)"), 2) + "\n";
    strUsage += "  -anonymizereduxamount=<n>     " + strprintf(_("Keep N REDUX anonymized (default: %u)"), 0) + "\n";
    strUsage += "  -liquidityprovider=<n>       " + strprintf(_("Provide liquidity to StealthX by infrequently mixing coins on a continual basis (0-100, default: %u, 1=very frequent, high fees, 100=very infrequent, low fees)"), 0) + "\n";
    ...
    strUsage += "  -rpcport=<port>        " + strprintf(_("Listen for JSON-RPC connections on <port> (default: %u or testnet: %u)"), 9666, 19666) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2014-%i The Redux Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // ********************************************************* Step 10: setup StealthX

    uiInterface.InitMessage(_("Loading masterx cache..."));

    CMasterXDB gmdb;
    CMasterXDB::ReadResult readResult = gmdb.Read(gmineman);
    ...
    uiInterface.InitMessage(_("Loading masterx payment cache..."));

    CMasterXPaymentDB gmpayments;
    CMasterXPaymentDB::ReadResult readResult3 = gmpayments.Read(masterxPayments);
    
    if (readResult3 == CMasterXPaymentDB::FileError)
        LogPrintf("Missing masterx payment cache - gmpayments.dat, will try to recreate\n");
    else if (readResult3 != CMasterXPaymentDB::Ok)
    {
        LogPrintf("Error reading gmpayments.dat: ");
        if(readResult3 == CMasterXPaymentDB::IncorrectFormat)
            LogPrintf("magic is ok but data has invalid format, will try to recreate\n");
        else
            LogPrintf("file format is unknown or invalid, please fix it manually\n");
    }

    fMasterX = GetBoolArg("-masterx", false);

    if((fMasterX || masterxConfig.getCount() > -1) && fTxIndex == false) {
        return InitError("Enabling MasterX support requires turning on transaction indexing."
                  "Please add txindex=1 to your configuration and start with -reindex");
    }

    if(fMasterX) {
        LogPrintf("IS STEALTHX MASTER NODE\n");
        strMasterXAddr = GetArg("-masterxaddr", "");

        LogPrintf(" addr %s\n", strMasterXAddr.c_str());

        if(!strMasterXAddr.empty()){
            CService addrTest = CService(strMasterXAddr);
            if (!addrTest.IsValid()) {
                return InitError("Invalid -masterxaddr address: " + strMasterXAddr);
            }
        }

        strMasterXPrivKey = GetArg("-masterxprivkey", "");
        if(!strMasterXPrivKey.empty()){
            std::string errorMessage;

            CKey key;
            CPubKey pubkey;

            if(!spySendSigner.SetKey(strMasterXPrivKey, errorMessage, key, pubkey))
            {
                return InitError(_("Invalid masterxprivkey. Please see documenation."));
            }

            activeMasterX.pubKeyMasterX = pubkey;

        } else {
            return InitError(_("You must specify a masterxprivkey in the configuration. Please see documentation for help."));
        }
    }
    
    //get the mode of evolution voting for this masterx
    strEvolutionMode = GetArg("-evolutionvotemode", "auto");

    if(GetBoolArg("-gmconflock", true) && pwalletMain) {
        LOCK(pwalletMain->cs_wallet);
        LogPrintf("Locking MasterX:\n");
        uint256 mnTxHash;
    ...
    fEnableStealthX = GetBoolArg("-enablestealthx", false);

    nStealthXRounds = GetArg("-stealthxrounds", 2);
    if(nStealthXRounds > 16) nStealthXRounds = 16;
    if(nStealthXRounds < 1) nStealthXRounds = 1;

    nLiquidityProvider = GetArg("-liquidityprovider", 0); //0-100
    if(nLiquidityProvider != 0) {
        spySendPool.SetMinBlockSpacing(std::min(nLiquidityProvider,100)*15);
        fEnableStealthX = true;
        nStealthXRounds = 99999;
    }

    nAnonymizeReduxcoinAmount = GetArg("-anonymizereduxamount", 0);
    if(nAnonymizeReduxcoinAmount > 999999) nAnonymizeReduxcoinAmount = 999999;
    if(nAnonymizeReduxcoinAmount < 2) nAnonymizeReduxcoinAmount = 2;
    ...
    //lite mode disables all MasterX and StealthX related functionality
    fLiteMode = GetBoolArg("-litemode", false);
    if(fMasterX && fLiteMode){
        return InitError("You can not start a masterx in litemode");
    }

    LogPrintf("fLiteMode %d\n", fLiteMode);
    LogPrintf("nInstantXDepth %d\n", nInstantXDepth);
    LogPrintf("StealthX rounds %d\n", nStealthXRounds);
    LogPrintf("Anonymize Redux Amount %d\n", nAnonymizeReduxcoinAmount);
    LogPrintf("Evolution Mode %s\n", strEvolutionMode.c_str());

    /* Denominations

       A note about convertability. Within StealthX pools, each denomination
    ...
    */
    ...
    threadGroup.create_thread(boost::bind(&ThreadCheckStealthXPool));
